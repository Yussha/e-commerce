        ...
        <div className={`${styles.customBackground} grid grid-rows-3 gap-y-8 p-3 sm:grid-rows-1 sm:grid-cols-2 h-auto sm:p-6 sm:items-center sm:gap-x-8 sm:gap-y-8 lg:grid-cols-3 `}>
            {bannersObj.map(banner => (
                <Link to="/shop" key={banner.id}>
                    <div onClick={() => setActiveLink("/shop")} className={`${styles.customBanner} ${styles[`customBanner-${banner.id}`]} h-64 flex-col sm:h-64 flex sm:flex-col items-center justify-center lg:h-72`}
                    >
